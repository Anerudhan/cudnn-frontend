...
#pragma unroll
        ...
        const uint64_t dz = gdesc16(R.z), kg_desc = gdesc64(R.kg);
        wg_fence();
        if (zero_start) {
#pragma unroll
            for (int rh = 0; rh < 2; ++rh) {
                wg64_zero(st[2 * rh + 0], ra[rh], kg_desc);
                wg64_zero(st[2 * rh + 1], ra[rh], kg_desc + 128);
            }
        } else {
#pragma unroll
            for (int rh = 0; rh < 2; ++rh) {
                wg16_add(out[rh], ra[rh], dz);
                wg64_add(st[2 * rh + 0], ra[rh], kg_desc);
                wg64_add(st[2 * rh + 1], ra[rh], kg_desc + 128);
            }
        }
        wg_commit();
